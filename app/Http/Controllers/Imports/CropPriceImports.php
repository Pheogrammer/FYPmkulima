...
class CropPricesImport implements ToCollection
{
    ...
    public function collection(Collection $rows)
    {
        ...
        // Fetch existing prices for the given crop and regions
        $existingPrices = Price::where('cropID', $this->cropID)
            ->whereIn('regionID', function ($query) use ($regions) {
                $query->select('id')
                    ->from('regions')
                    ->whereIn('name', $regions);
            })
            ->get();

        // Delete the matching regionID entries in the prices table
